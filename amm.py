...
class amm:
    ...
    def show(self):
        for P in self.parties:
            P.show()
        print("AMM [" + f(self.r_0) + ":t_0," + f(self.r_1) + ":t_1], v = " + f(self.get_v()) + " $, t_0*t_1 = " + f(self.r_0*self.r_1) + "\n")
...
# Example from https://arxiv.org/pdf/2106.01870.pdf

# Values of t_0 and t_1 in $ (for simplicity both equal to 1000 $ per unit)
v_t_0 = 1000
v_t_1 = 1000
...
# Initial value of potential victim tokens
v_A_pre = A.get_v()

# Initial value of attacker tokens
v_M_pre = M.get_v()

# True if attack has to be executed
attack = True
...
if attack: 
    ...
    # Compute and print profit of the attacker M after front-run
    print("Profit of M afer front-run = " + f(M.get_v() - v_M_pre) + "$")

print("\n# Potential victim swap\n")
AMM.sl(A,20,15)

# Compute and print profit of the victim A
print("Profit of A after swap = " + f(A.get_v() - v_A_pre) + "$")

if attack: 
    print("\n# Back-run\n")
    AMM.sr(M,25.9,20.6)

    # Compute and print profit of the attacker M after back-run
    print("Profit of M after back-run = " + f(M.get_v() - v_M_pre) + "$")
